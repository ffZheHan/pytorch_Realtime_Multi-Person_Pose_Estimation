...
def tags2COCOIndex(listC):
    return [COCOdict[item] for item in listC]
...
class Normalize(Preprocess):
    ...
    def normalize_annotations(anns):
        ...
        for ann in anns:
            ann_nodes = ann['nodes']
            for idx, node in enumerate(ann_nodes):
                if node == 'null' or node == None:
                    # print("node0 = {0}".format(node))
                    ann_nodes[idx] = [-1, -1]
                # print("node1 = {0}".format(node))

            # ann['key_points'] = []
            # print("ann_nodes = {0}".format(ann_nodes))
            # for idx , value in enumerate(ann_nodes):
            #     ann['key_points'].append([*value, COCOdict[ann['tags'][idx]]])
            #     print("key_points.{}".format(ann['key_points']))

            ann['keypoints'] = []
            
            for idx , value in enumerate(ann_nodes):
                ann['keypoints'].append([*value, COCOdict[ann['tags'][idx]]])
                print("keypoints.{}".format(ann['keypoints']))
...
class RescaleRelative(Preprocess):
    ...
    def scale(self, image, anns, factor):
        ...
        for ann in anns:
            print("keypoints_anns={0}".format(ann['keypoints']))
