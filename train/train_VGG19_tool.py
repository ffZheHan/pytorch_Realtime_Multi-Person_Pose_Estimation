...
from lib.datasets import tool, transformstool, datasets_tool
...
# DATA_DIR = '/data'
# DATA_DIR = '/data/robustmislite'
DATA_DIR = '/nfs/home/zhan/code/PoseEstimate/pytorch_Realtime_SurgicalTool_Pose_Estimation/data/robustmislite'

# coco dataset
# ANNOTATIONS_TRAIN = [os.path.join(DATA_DIR, 'annotations', item) for item in ['tool_keypoints_train.json']]
# ANNOTATIONS_VAL = os.path.join(DATA_DIR, 'annotations', 'tool_keypoints_val.json')
# IMAGE_DIR_TRAIN = os.path.join(DATA_DIR, 'images/train')
# IMAGE_DIR_VAL = os.path.join(DATA_DIR, 'images/val')

# how to split our dataset into 8:2 
image_files = sorted(glob.glob(os.path.join(DATA_DIR,'**', '1', '**','raw.png'), recursive=True))
annotation_files = sorted(glob.glob(os.path.join(DATA_DIR,'**','1','**','raw.json'), recursive=True))
correspondinglist = []
for elem1,elem2 in zip(image_files, annotation_files):
    correspondinglist.append((elem1, elem2))
train_files, val_files = train_test_split(correspondinglist, test_size=0.2, random_state=42)

print("Train dataset size:", len(train_files))
print("Validation dataset size:", len(val_files))

ANNOTATIONS_TRAIN = [item[1] for item in train_files] 
ANNOTATIONS_VAL = [item[1] for item in val_files] 
IMAGE_DIR_TRAIN = [item[0] for item in train_files] 
IMAGE_DIR_VAL = [item[0] for item in val_files] 
...
def train_cli(parser):
    ...
    group.add_argument('--pre-n-images', default=80, type=int,
                       help='number of images to sampe for pretraining')
...
def train_factory(args, preprocess, target_transforms):
    ...
    train_datas = [datasets_tool.Surgicaltool(
        root_dir=args.train_image_dir,
        preprocess=preprocess,
        image_transform=transformstool.image_transform_train,
        target_transforms=target_transforms,
    ) for item in args.train_annotations]
    ...
    val_data = datasets_tool.Surgicaltool(
...
# load train data
preprocess = transformstool.Compose([
        transformstool.Normalize(),
        transformstool.RandomApply(transformstool.HFlip(), 0.5),
        transformstool.RescaleRelative(),
        transformstool.Crop(args.square_edge),
        transformstool.CenterPad(args.square_edge),
    ])
